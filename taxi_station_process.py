...
if __name__ == "__main__":
    # Taksi durakları için dosya adı ve durak adı sütunu ipucu
    points_data_file = "taxi_station.json"
    point_name_column_hint = "DURAK_ADI" # taxi_station.json içindeki durak adı sütunu
    output_prefix = "taxi_duraklari"
    ...
    if points_with_mahalle_gdf is not None and not points_with_mahalle_gdf.empty:
        ...
        # Sadece nokta adı (eğer varsa) ve mahalle adını gösterelim
        display_cols = []
        ...
        else: # Eğer sütun adları bulunamazsa tümünü göster
            print(points_with_mahalle_gdf.head(20).to_string())


        # Atanamayan nokta sayısını kontrol et
        if assigned_mahalle_col_name and assigned_mahalle_col_name in points_with_mahalle_gdf.columns:
            unassigned_points_count = points_with_mahalle_gdf[assigned_mahalle_col_name].isnull().sum()
            print(f"\nToplam {unassigned_points_count} nokta (durak/istasyon) herhangi bir mahalleye atanamadı (mahalle bilgisi boş).")
        else:
            print("\nMahalle adı sütunu çıktıda bulunamadı, atanamayan nokta sayısı kontrol edilemiyor.")

        # Çıktı dosyalarının adlarını güncelle
        ...
        try:
            # Geometri sütununu CSV'ye yazmak yerine, sadece ilgili bilgileri yazalım
